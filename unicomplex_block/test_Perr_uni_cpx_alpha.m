...
% legend('Pe ML rule','Pe,union sim ML','Pe,union theory ML',...
%     'Pe corr rule','Pe,union sim corr','Pe,union theory corr');
...
% hold on; grid on;
% semilogy(mm,1-(1-sum(perramargin_ML,2)).*(1-ed),'m*');
% semilogy(mm,1-(1-perrcorr).*(1-ed),'b--');
% semilogy(mm,1-(1-sum(perramargin,2)).*(1-ed),'m+');
...
if (IS_PREDICT_ONLY >= 2)
    ...
    dtt(dtt==':')='-';
    save(sprintf('tpn_ALPHA_N%d_rho_tot%ddB_alphaPower%d_1e%d_%s.mat',N,rho_tot_dB,alpha,-log10(epsilon),dtt));
end