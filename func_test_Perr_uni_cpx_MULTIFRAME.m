function func_test_Perr_uni_cpx_MULTIFRAME(N,rho_tot_dB,alpha,NBFRAME,NTEST)
% nopt = 90;mopt = 12;
% N = nopt+mopt;
% rho_tot_dB = -1;
% % rhodB = 0;%0; %3; %rho_d
...
%     perr2v2_trunc(im) = perr_uni_cpx_bloc(2*m,2*n,ssv2{im}(1:end-1),N/(m*alpha+n)*rho_tot, CONST_ML_RULE, NTEST);
    ...
    perr2_iterative1(im) = perr_uni_cpx_iterative_bloc(m,n,ss{im},rhoD_tab(im), CONST_ML_RULE, NTEST, 1,NBFRAME);
%     perr2_iterative2(im) = perr_uni_cpx_iterative_bloc(m,n,ss{im},rhoD_tab(im), CONST_ML_RULE, NTEST, 2,NBFRAME);
    perr2_iterative4(im) = perr_uni_cpx_iterative_bloc(m,n,ss{im},rhoD_tab(im), CONST_ML_RULE, NTEST, 4,NBFRAME);
    %real Pe iterative CORR rule
    perr2_cor_iterative1(im) = perr_uni_cpx_iterative_bloc(m,n,ss{im},rhoD_tab(im), CONST_COR_RULE, NTEST, 1,NBFRAME);
%     perr2_cor_iterative2(im) = perr_uni_cpx_iterative_bloc(m,n,ss{im},rhoD_tab(im), CONST_COR_RULE, NTEST, 2,NBFRAME);
    perr2_cor_iterative4(im) = perr_uni_cpx_iterative_bloc(m,n,ss{im},rhoD_tab(im), CONST_COR_RULE, NTEST, 4,NBFRAME);
